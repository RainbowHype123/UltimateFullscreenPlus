...
PROPS_DATA_START()
	...
	//PropData_Group(PROPID_NUM1, PSTR("Play Area"), PSTR("This is the safe area resolution that will never be cropped when using Fullscreen type 6.")),
	//PropData_EditNumber(PROPID_NUM2,PSTR("Width"),PSTR("")),
	//PropData_EditNumber(PROPID_NUM3, PSTR("Height"), PSTR("")),
...
void WINAPI DLLExport GetObjectRect(mv _far *mV, RECT FAR *rc, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	//rc->right  = rc->left + 32;
	//rc->bottom = rc->top  + 32;
	rc->right  = rc->left + max(edPtr->swidth,32);
	rc->bottom = rc->top  + max(edPtr->sheight,32);
#endif // !RUN_ONLY
	return;
}
...
// This will draw either a icon or a rectangle to the size of the object.
/*void WINAPI DLLExport EditorDisplay(mv _far* mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR* rc)
{
#ifndef RUN_ONLY
	...
	//Draw shapes
	ps->Rectangle(rc->left, rc->top, rc->right, rc->bottom, &mosaic, 1, &outline, 0, BMODE_TRANSP, BOP_BLEND, 64);
