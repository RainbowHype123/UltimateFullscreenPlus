...
bool GlobalDataInitialized = false;
...
short WINAPI DLLExport CreateRunObject(LPRDATA rdPtr, LPEDATA edPtr, fpcob cobPtr)
{
  ...
  //Before we initialize our data values, we need to figure out where the window already is,
  //and whether or not it's maximized already.
  ...
  HWND framewindow = rhPtr->rhHMainWin;
  ...
  //We need to re-size the screen every frame change
  GlobalData.fullscreen_initialized = false;

  if(!GlobalDataInitialized)
  {
	//GlobalData.fullscreen_initialized = false;
	GlobalData.fullscreen = false;  //IsZoomed(framewindow)!=0;   //NOTE: because the window only gets zoomed AFTER this function is called,
                                                            //we need to initialize it in HandleRunObject instead.
	GlobalData.apply_fs_settings_to_window = false;
	GlobalData.listen_for_maximize = true;
	GlobalData.listen_for_monitor_switch = true;


	GlobalData.default_gamerectx = winrect.left;
	GlobalData.default_gamerecty = winrect.top;
	GlobalData.default_gamerectwidth = winrect.right-winrect.left;
	GlobalData.default_gamerectheight= winrect.bottom-winrect.top;
	...
	GlobalData.fstype = BESTFIT;
	GlobalData.gamescale = 1.0f;

	GlobalData.PlayX = 640;
	GlobalData.PlayY = 480;
...
{
	bool isZoomed = (IsZoomed(rdPtr->rHo.hoAdRunHeader->rhHMainWin)!=0);


	//We need to initialize "fullscreen" here, because it the window isn't maximized yet in the CreateRunObject function.
	if(!GlobalData.fullscreen_initialized)
	{
		GlobalData.fullscreen_initialized = true;
		GlobalData.fullscreen = isZoomed;

		UpdateGameRectangleSize(rdPtr);
	}

	if (GlobalData.listen_for_monitor_switch == true)
	{
		HMONITOR current_monitor = MonitorFromWindow(rdPtr->rHo.hoAdRunHeader->rhHMainWin, MONITOR_DEFAULTTONULL);
		if (GlobalData.previous_monitor != current_monitor)
		{
			rdPtr->rRd->GenerateEvent(5);  //trigger "on monitor switch" event

			if (GlobalData.fullscreen)
			{
				UpdateGameRectangleSize(rdPtr);
				rdPtr->rRd->Rehandle();
				GlobalData.previous_monitor = current_monitor;
			}
		}
	}
...
*/
	...
	if(!GlobalData.listen_for_maximize)   //then there's no reason for us to keep running this loop
		return REFLAG_ONESHOT;

	if(GlobalData.fullscreen != isZoomed)
	{
		GlobalData.fullscreen = isZoomed;
		UpdateGameRectangleSize(rdPtr);
	...
	//return REFLAG_ONESHOT;//REFLAG_ONESHOT;

	//Actually, we need to constantly monitor whether the screen's maximized state has been changed outside of this object, so continue to run this.
	return 0;
